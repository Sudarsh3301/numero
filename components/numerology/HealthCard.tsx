  ...
  return (
    <div className="bg-white/[0.04] rounded-organic p-3.5">
      <div className="flex gap-3 items-center mb-1">
        <div className="flex-shrink-0 w-11 h-11 rounded-card flex items-center justify-center text-xl bg-white/5 border border-white/10">
          🏥
        </div>
        <div className="text-[12px] font-bold text-white/90">Health Profile · {label}</div>
      </div>
      ...
      <div className="mb-2">
        ...
        {health.issues.slice(0, 3).map((issue, i) => (
          <div key={i} className="text-[10px] text-warning mb-0.5">• {issue}</div>
        ))}
      </div>

      <div className="bg-success/10 rounded-lg p-2">
        <div className="text-[9px] text-success font-bold mb-0.5">Lifestyle Advice:</div>
